    ...
    return (
        <div style={{ borderColor: props.lightmode ? "#bfcbd3" : "#4b5563" }} className='flex gap-3 w-full p-3 border-2 rounded-l'>
            ...
            <div className='flex gap-3 w-1/2 items-center relative justify-start'>
                ...
                <input maxLength={20} disabled={props.loading || loading} className='border-2 h-10 border-blue-200 w-32 outline-none font-thin text-gray-600 rounded-lg px-2 focus:border-gray-600' value={props.chatName} onChange={e => props.setChatName(e.target.value)} placeholder='Chat Name' />
            ...
            <div className="flex gap-2 w-1/2 justify-end items-center">
                {props.people.map((person, index) =>
                    <button disabled={props.loading} onClick={() => props.setIndexPerson(index)} style={{ backgroundColor: index == props.indexPerson ? "#be185d" : "#f8fafc" }} key={index} className="w-12 rounded-full bg-amber-500 p-1 cursor-pointer hover:bg-slate-400 duration-200 shadow hover:shadow-2xl hover:shadow-amber-500">
            ...
            {/* bg-[#f3f1f12b] */}
            {dialog ? <dialog style={{ visibility: dialog ? 'visible' : "hidden" }} id="dialog-chatimage" className='w-screen h-screen flex flex-col justify-center items-center rounded-xl p-4 absolute top-0 bg-[#2f2e2ead] backdrop-blur-xl z-50'>
