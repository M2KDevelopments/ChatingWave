...
function FacebookConversation(props: IPhone) {
    ...
    return (
        <div id={`conversation-${props.id}`} aria-label="conversation" className='w-full flex flex-col gap-2 p-3 bg-white' style={{ overflowY: props.noScrollBar ? "hidden" : "scroll", height: props.height * heightPercentage }}>
            {
                props.messages.map((msg, index) =>
                    <div
                        ...
                        className='relative text-gray-900 max-w-[60%] rounded-lg flex flex-col p-2'
                        id={`msg-${props.id}-${msg.id}`} // msg-preview-{id} or msg-phone-{id}
                        key={msg.id + index}>

                        {/* Reply UI */}
                        {msg.replyId && messageMap.get(msg.replyId) ?
                            <div style={{ left: msg.me ? undefined : profileSize*1.5 }} className='bg-[#f7f7f7] border-l-4 rounded-2xl flex w-4/5 py-2 relative top-2'>
                        ...
                        {msg.me ? <p style={{
                            ...
                            background: msg.me ? "color-mix(in srgb, #a201f3 50%, #087aff 50%)" : "#f0f0f0",
                        ...
                        <div className='w-full relative'>
                            {msg.reactions.length ?
                                <div style={{ fontSize: fontSize, left: msg.me ? undefined : 40, right: msg.me ? 4 : undefined }} className='w-fit absolute -bottom-5 rounded-full px-1 bg-white shadow-sm flex justify-center items-center gap-1'>
